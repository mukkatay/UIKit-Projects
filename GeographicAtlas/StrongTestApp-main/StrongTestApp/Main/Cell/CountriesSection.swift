...
struct CountryResponse: Codable {
  ...
  func getMappedCountry() -> (country: Country, region: String) {
    return (
      Country(
        name: name?.common ?? "",
        capital: capital?.first ?? "",
        capitalCoord: latlng?.compactMap{ String($0) }.joined(separator: ", ") ?? "",
        flag: flags?.png ?? "",
        population: String(population ?? 0),
        area: String(area ?? 0),
        currencies: currencies?.currenciesTRY?.name ?? "",
